...
export default function Gallery() {
  ...
  const [cloudItems, setCloudItems] = useState<Item[]>([])

  useEffect(() => {
    const cloudName = import.meta.env.VITE_CLOUDINARY_CLOUD_NAME
    if (!cloudName) return
    fetch(`https://res.cloudinary.com/${cloudName}/image/list/leitech-gallery.json`)
      .then(r => r.json())
      .then(data => {
        const mapped: Item[] = (data.resources ?? []).map((r: { public_id: string; secure_url: string; context?: { custom?: Record<string, string> } }) => {
          const ctx = r.context?.custom ?? {}
          return { file: r.public_id, url: r.secure_url, location: ctx.location ?? 'Ghana', tag: (ctx.service_tag ?? 'Solar') as Tag, spec: ctx.spec }
        })
  ...
  const allItems = useMemo(() => [...cloudItems, ...items], [cloudItems])
    ...
    <section id="gallery" className="section section--alt">
      <div className="container">
        ...
        <div className="gal-filters rev">
          {FILTERS.map(f => {
            const count  = f === 'All' ? allItems.length : allItems.filter(i => i.tag === f).length
        ...
        <div className="rev gal-grid">
          {paged.map((item, i) => {
            ...
            return (
              ...
              >
                <img
                  src={item.url ?? `/images/${encodeURIComponent(item.file)}`}
                  alt={item.location}
      ...
      {activeItem && (
        ...
        >
          ...
          <img
            className="lb-img"
            src={activeItem.url ?? `/images/${encodeURIComponent(activeItem.file)}`}
